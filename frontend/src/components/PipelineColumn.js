...
export default function PipelineColumn({ title, leads, statuses, onUpdateStatus, onUpdateVisitDate, onUpdateAssignedTo, pendingVisitLeadId }) {
  return (
    <section className="min-w-[320px] rounded-3xl border border-slate-200 bg-slate-50/50 p-6 shadow-sm">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-900">{title}</h2>
          <p className="mt-1 text-sm text-slate-500">{leads.length} lead{leads.length !== 1 ? 's' : ''}</p>
        </div>
